...
| 页面 | 用途 |
| --- | --- |
| `/order/?roomid=房间号&source=obs` | OBS 浏览器源唯一播放页 |
| `/order/?roomid=房间号&livemode=false&source=control` | 控制/监控页，用于切歌、歌单、声音和队列控制 |

OBS 中添加“浏览器”来源时使用第一个链接；日常管理使用第二个链接。一个房间只需要这一组播放页和控制页，普通浏览器即使误打开 OBS 播放链接，也会在检测到已有 OBS 播放端后自动降级为监控页，不会再另起一套歌单。设置页可以从启动器打开，也可以访问 `/order/settings.html?roomid=房间号`。

如果添加到 Bilibili 直播姬，优先使用启动器生成的 `127.0.0.1` 地址，不要手动改回 `localhost`。直播姬部分版本的内置 WebView 对 `localhost` 回环解析和配置脚本执行顺序兼容性较差。
...
- OBS 页面：带 `source=obs`，给 OBS 浏览器源使用，负责真正播放音频，但画面只显示点歌队列列表，不显示播放器卡片和控制按钮。
- 控制页：带 `livemode=false&source=control`，用于控制播放、队列、空闲歌单和声音；不会另起一套弹幕点歌状态。
- 设置页：`settings.html`，只负责配置，不会自动播放歌曲。
- 启动器：`launcher.html`，输入房间号后生成播放页和控制页链接。

播放页和控制页的播放状态以本地服务端为唯一来源，不依赖浏览器的 localStorage 或是否属于同一个浏览器进程。服务端会为每个房间维护唯一播放端租约；无论打开多少个 OBS 播放链接或控制链接，歌曲、队列、当前歌单和播放状态都会同步到同一份房间状态。
