...
1. **没有实现 B 站当前官方播放器使用的消息序号回执。** 官方直播播放器在 `onReceivedMessage(messages, sequence)` 中发现 `sequence > 1` 时，会向 `/xlive/open-interface/v1/dm/message_ack` POST `{ terminal: 0, sequence }`。本项目读取了弹幕包头的 `sequence`，但既没有把它作为可靠消息序号记录，也没有发送 ACK。这是当前实现与官方协议最重要的差异，应优先验证和补齐。
2. **匿名 HTTP 会话和 WebSocket 会话没有统一。** `lib_piliplus` 即使未登录，也会生成并持久化 `buvid3`，所有 HTTP 请求通过同一个 CookieJar；当前 Node 端获取导航信息、弹幕 token、历史弹幕时没有持久 Cookie 会话，WebSocket 又固定使用 `uid=0`。日志里 `uid: 0` 和打码用户名首先是游客隐私字段降级，不等于这条弹幕本身丢失，但会话不一致可能影响鉴权、ACK 和服务端分发策略。
...
因此修复顺序应当是：先加可观测性确认消息在哪一层消失，再实现 ACK 和一致匿名会话，然后改为单房间连接中心，最后补半包缓存、断线补偿和去重。不能只继续增加浏览器端 `console.log`，因为现在的日志只记录已经成功解析出来的 `DANMU_MSG`，无法证明 B 站有没有发、代理有没有丢、解析有没有跳过。
...
本项目目前只把 16 字节协议包头的 sequence 打进调试信息，没有建立 ACK 流程。实施时不能武断地对每个心跳序号回执；必须先在原始事件中记录官方回调所对应的可靠消息 sequence，确认其来源和单调性，再只对 `sequence > 1` 的消息进行 ACK。
...
- `nav`、房间号解析、`getDanmuInfo`、历史接口和 `message_ack` 全部复用同一会话；
...
由房间连接中心使用自己的 `BiliSession` 获取真实房间号、token 和 host。这样 token、Cookie、ACK 和重连都属于同一后端会话。
...
ACK 功能先放在仅 `realtime=1` 生效的内部开关下。连续对账确认漏收率下降且没有异常限流后，再作为实时模式默认行为。
...
5. 观察到可靠 sequence 时 ACK 成功率达到 99% 以上，失败有重试且不阻塞弹幕处理；
